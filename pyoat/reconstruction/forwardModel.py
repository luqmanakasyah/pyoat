...
class modelOA:
    ...
    def calculateProjection(self, xPoint, yPoint, rPoint, theta, nRows, i):
        ...
        xPointUnrotated = xPoint * np.cos(theta) - yPoint * np.sin(theta)
        # horizontal position of the points of the curve in the original grid (not rotated)
        yPointUnrotated = xPoint * np.sin(theta) + yPoint * np.cos(theta)
        # vertical position of the points of the curve in the original grid (not rotated)
        ...
        distToOrigin = np.sqrt(
            np.power((xPaddedRight - xPaddedLeft), 2)
            + np.power((yPaddedRight - yPaddedLeft), 2)
        )
        ...
        vecIntegral = (1 / 2) * np.divide(
            (
                np.concatenate((distToOrigin, np.zeros((1, lt))))
                + np.concatenate((np.zeros((1, lt)), distToOrigin))
            ),
            rPoint,
        )  # vector for calculating the integral
        del distToOrigin

        xNorm = (
            xPointUnrotated + (reconDimsXY / 2)
        ) / pixelSize + 1  # horizontal position of the points of the curve in normalized coordinates
        del xPointUnrotated

        yNorm = (
            yPointUnrotated + (reconDimsXY / 2)
        ) / pixelSize + 1  # vertical position of the points of the curve in normalized coordinates
        del yPointUnrotated

        xBefore = np.floor(
            xNorm
        )  # horizontal position of the point of the grid at the left of the point (normalized coordinates)
        xAfter = np.floor(
            xNorm + 1
        )  # horizontal position of the point of the grid at the right of the point (normalized coordinates)
        xDiff = xNorm - xBefore
        del xNorm

        yBefore = np.floor(
            yNorm
        )  # vertical position of the point of the grid below of the point (normalized coordinates)
        yAfter = np.floor(
            yNorm + 1
        )  # vertical position of the point of the grid above of the point (normalized coordinates)
        yDiff = yNorm - yBefore
        del yNorm
        ...
        inPoint1 = (
            (xSquare1 > 0)
            & (xSquare1 <= resolutionXY)
            & (ySquare1 > 0)
            & (ySquare1 <= resolutionXY)
        )  # boolean to decide 1. point of the square is inside the grid
        inPoint2 = (
            (xSquare2 > 0)
            & (xSquare2 <= resolutionXY)
            & (ySquare2 > 0)
            & (ySquare2 <= resolutionXY)
        )  # boolean to decide 2. point of the square is inside the grid
        inPoint3 = (
            (xSquare3 > 0)
            & (xSquare3 <= resolutionXY)
            & (ySquare3 > 0)
            & (ySquare3 <= resolutionXY)
        )  # boolean to decide 3. point of the square is inside the grid
        inPoint4 = (
            (xSquare4 > 0)
            & (xSquare4 <= resolutionXY)
            & (ySquare4 > 0)
            & (ySquare4 <= resolutionXY)
        )  # boolean to decide 4. point of the square is inside the grid
        ...
        pos1 = (
            resolutionXY * (xSquare1 - 1) + ySquare1
        )  # one dimensional position of the first points of the squares in the grid
        pos2 = (
            resolutionXY * (xSquare2 - 1) + ySquare2
        )  # one dimensional position of the first points of the squares in the grid
        pos3 = (
            resolutionXY * (xSquare3 - 1) + ySquare3
        )  # one dimensional position of the first points of the squares in the grid
        pos4 = (
            resolutionXY * (xSquare4 - 1) + ySquare4
        )  # one dimensional position of the first points of the squares in the grid
        del (
            xSquare1,
            xSquare2,
        ...
        weight1 = (
            (1 - xDiff) * (1 - yDiff) * vecIntegral
        )  # weight of the first point of the triangle
        weight2 = (
            (xDiff) * (1 - yDiff) * vecIntegral
        )  # weight of the second point of the triangle
        weight3 = (
            (1 - xDiff) * (yDiff) * vecIntegral
        )  # weight of the third point of the triangle
        weight4 = (
            (xDiff) * (yDiff) * vecIntegral
        )  # weight of the fourth point of the triangle
        ...
        rowMatrix = np.transpose(
            np.transpose(np.expand_dims(np.linspace(1, lt, lt, dtype=int), axis=0))
            * np.ones((1, nAngles), dtype=int)
        )  # rows of the sparse matrix
        rowMatrixVec = (
            np.transpose(np.transpose(rowMatrix).reshape(-1, 1)) - 1
        )  # rows of the sparse matrix in vector form
        del rowMatrix

        rowMat = np.concatenate(
            (
                rowMatrixVec[inVec1] + (i * lt),
                rowMatrixVec[inVec2] + (i * lt),
                rowMatrixVec[inVec3] + (i * lt),
                rowMatrixVec[inVec4] + (i * lt),
            )
        )

        posMat = np.concatenate(
            (
                posVec1[inVec1] - 1,
                posVec2[inVec2] - 1,
                posVec3[inVec3] - 1,
        ...
        projectionMatrix = csc_matrix(
            (weightMat, (rowMat, posMat)), shape=(nRows, nCols)
        )
    ...
    def calculateModel(self, timePoints):
        ...
        angleMax = np.arcsin(
            ((reconDimsXY + 2 * pixelSize) * np.sqrt(2)) / (2 * rSensor)
        )
        ...
        angles = np.transpose(
            np.expand_dims(np.linspace(-angleMax, angleMax, nAngles), axis=0)
        ) * np.ones((1, len(timePoints)))
        ...
        for i in range(0, len(angleSensor)):
            ...
            rMinus = (
                np.ones((nAngles, 1)) * minusDistSensor
            )  # -t distance from sensor to curve
            rPlus = (
                np.ones((nAngles, 1)) * plusDistSensor
            )  # +t distance from sensor to curve

            xMinust = rSensor[i] - (rMinus) * np.cos(
                angles
            )  # x distance at -t based on (0,0) to transducer coordinate system
            yMinust = (rMinus) * np.sin(
                angles
            )  # y distance at +t based on (0,0) to transducer coordinate system
            ...
            projectionMinust = self.calculateProjection(
                xMinust, yMinust, rMinus, theta, nRows, i
            )
            projectionPlust = self.calculateProjection(
                xPlust, yPlust, rPlus, theta, nRows, i
            )

            if i > 0:
                modelMatrix = modelMatrix + (1 / (2 * dt)) * (
                    projectionPlust - projectionMinust
                )
            else:
